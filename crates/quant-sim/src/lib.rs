...
//!
//! # Why latency changes the answer and not just the timing
//!
//! An order submitted at `T` reaches the venue at `T + outbound`, and the book
//! it walks is the book *then*, not the one that prompted it. That is where
//! slippage comes from in this simulator, and it is why there is no separate
//! slippage model: the price we get is the price that was there when we arrived,
//! which is the whole of the phenomenon at our size.
//!
//! Inbound latency is not symmetric with it. Outbound changes **what price we
//! get**; inbound changes **when we find out**, and therefore what information
//! the strategy's next decision is made on. A strategy that reconciles its
//! position behaves differently under the two, which is why they are separate
//! numbers.
//!
//! A fill's timestamp stays its *generation* time. That is when it happened at
//! the venue, which is what a venue reports; we simply learn later. Stamping it
//! with the delivery time would make our own latency look like the market's.
//!
...
struct Resting {
    ...
    /// Size not yet filled.
    remaining: Qty,
    /// When the venue has it. Before this, it does not exist as far as the
    /// market is concerned: it cannot fill, and it has not been acknowledged.
    arrives_at: Ts,
...
pub struct SimStats {
    ...
    pub no_market: u64,
    /// Reports still in flight when the data ran out.
    ///
    /// Not zero once inbound latency is on: an event generated by the last
    /// event of the run has nowhere to be delivered. Reported rather than
    /// flushed, because flushing would mean telling the strategy something it
    /// could not have known -- and a fill left undelivered leaves the portfolio
    /// holding a position it had actually closed, which is exactly the kind of
    /// discrepancy that should be visible.
    pub undelivered: u64,
...
pub struct SimulatedVenue {
    resting: Vec<Resting>,
    out: Vec<Pending>,
    /// Cancel requests in flight, with the time each reaches the venue.
...
impl SimulatedVenue {
    ...
    pub fn stats(&self) -> SimStats {
        SimStats {
            undelivered: self.out.len() as u64,
            ..self.stats
        }
    }
    ...
    /// Queue a report, to be delivered after the inbound latency.
    fn report(&mut self, now: Ts, event: ExecutionEvent) {
        self.out.push(Pending {
            deliver_at: Ts::from_nanos(now.as_nanos() + self.costs.latency.inbound),
            event,
        });
    }
    ...
    /// Report an order gone, with whatever never traded.
    fn cancelled(&mut self, now: Ts, id: ClientOrderId, remaining: Qty) {
    ...
    /// Apply cancel requests that have reached the venue.
    ///
    /// A cancel for an order that already filled finds nothing, and says
    /// nothing. Not an oversight: the cancel lost the race, and the fill it lost
    /// to has already been reported. Inventing a `Cancelled` here would tell a
    /// strategy its order was withdrawn when it had actually traded.
    fn apply_cancels(&mut self, now: Ts) {
        let mut still_flying = Vec::new();
        for (due, id) in core::mem::take(&mut self.cancels) {
            if due > now {
                still_flying.push((due, id));
                continue;
            }
            if let Some(i) = self.resting.iter().position(|o| o.id == id) {
                let order = self.resting.remove(i);
                self.cancelled(now, id, order.remaining);
            }
        }
...
impl ExecutionVenue for SimulatedVenue {
    ...
    fn cancel(&mut self, client_order_id: ClientOrderId, now: Ts) {
        // Queued, not applied. A cancel travels the same wire an order does, and
        // pretending otherwise would make cancelling free -- which is exactly
        // the assumption that makes a market-making backtest look safe.
        self.cancels.push((
            Ts::from_nanos(now.as_nanos() + self.costs.latency.outbound),
            client_order_id,
        ));
    }

    fn observe(&mut self, event: &MarketEvent, book: &Book, now: Ts) {
        self.apply_cancels(now);
        ...
        for order in &mut orders {
            // Still in flight: the venue does not have it, so it cannot fill and
            // has not been acknowledged. With zero latency this is never true,
            // because `observe` has already run for the event the order was
            // submitted on.
            ...
            if !order.announced {
                ...
                self.report(
                    ...
                    ExecutionEvent::Accepted {
                        ...
                        // inventing one would make backtest output look like it
                        // came from an exchange.
                        venue_order_id: None,
                        ts: now,
                    },
                );
    ...
    fn poll(&mut self, now: Ts, out: &mut Vec<ExecutionEvent>) {
        // Only what has arrived. Anything still in flight stays put, which is
        // what makes inbound latency mean something rather than being a field.
        let mut still_flying = Vec::new();
        for pending in core::mem::take(&mut self.out) {
            if pending.deliver_at <= now {
                out.push(pending.event);
            } else {
                still_flying.push(pending);
            }
        }
        self.out = still_flying;
    }
}
